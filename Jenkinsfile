pipeline {
   ...
   environment {
     ...
     // REPOSITORY_GITHUB
     REPOSITORY_TAG="${ORGANIZATION_NAME}/${REPOSITORY_BLUE}:${BUILD_ID}"
     registry = "${ORGANIZATION_NAME}/${REPOSITORY_BLUE}"
   ...
   stages {
      stage('Preparation') {
         steps {
            cleanWs()
            git credentialsId: 'GitHub', url: "https://github.com/${ORGANIZATION_NAME}/${REPOSITORY_BLUE}"
         }
      }
